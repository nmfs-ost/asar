# Authors and Citation

## Authors

- **Samantha Schiano**. Author, maintainer.
  [](https://orcid.org/0009-0003-3744-6428)

- **Sophie Breitbart**. Author, contributor.
  [](https://orcid.org/0000-0001-9641-9786)

- **Steve Saul**. Author, contributor.

- **Kelli Johnson**. Contributor.
  [](https://orcid.org/0000-0002-5149-451X)

- **Bai Li**. Contributor. [](https://orcid.org/0000-0002-8249-1442)

## Citation

Source:
[`DESCRIPTION`](https://github.com/nmfs-ost/asar/blob/v2.0.6/DESCRIPTION)

Schiano S, Breitbart S, Saul S (2026). *asar: Build NOAA Stock
Assessment Report*. R package version 2.2.0.9000,
<https://github.com/nmfs-ost/asar>.

    @Manual{,
      title = {asar: Build NOAA Stock Assessment Report},
      author = {Samantha Schiano and Sophie Breitbart and Steve Saul},
      year = {2026},
      note = {R package version 2.2.0.9000},
      url = {https://github.com/nmfs-ost/asar},
    }
