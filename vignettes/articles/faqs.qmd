...
Yes! You can run `create_template()` without a converted model results file (hence why running the function without any arguments still produces a basic report template). This means that you can use `asar` to make assessments that are not model-based.

In this case, you can disregard the preamble chunk (its label is 'output_and_quantities'). Ensure that its "eval" chunk option is set to false ('eval: false').
...
Yes! You can convert your own output into the expected standardized format without `stockplotr::convert_output()`, by following the guidance in the ["Standardizing Assessment Model Output" `stockplotr` vignette](https://nmfs-ost.github.io/stockplotr/articles/convert_output_details.html).

## Where are the acronyms for each office (for the `authors` argument)?
Run `asar::affiliation_info` to see a list of the acronyms for each office.
...
No, only the acronyms you use in your report will appear in your report's glossary.

## Can I render individual child documents, rather than the skeleton?

You can render child documents (e.g., "01_introduction.qmd") as HTML files. These files will show you if your code is working, but not what the document will look like in your final report as it lacks the formatting specified in your skeleton's YAML.

As a workaround, we recommend making some temporary edits to the skeleton. You could alter irrelevant code chunks by 1) commenting them out or 2) setting `eval: false` in those chunks' options. Then, once you render the skeleton, the report's formatting will be preserved.

You could manually add a YAML specifying the format as PDF into the child document, but this would not show the report's full formatting and would have to be removed before rendering the report skeleton.

## What arguments can I use when trying to rerender my skeleton?

Rerendering a skeleton using `rerender = TRUE` can be a helpful and reproducible way to making edits to your skeleton document and leaving a record of your process for future you and future assessment scientists. The current arguments that can be called to adjust your skeleton file when `rerender = TRUE` are:
...
We plan to include future functionality to also include bib_file and spp_image as functional arguments when using `rerender = TRUE`.
...
You will notice that the order of your figures and tables in their respective 08_figures.qmd and 09_tables.qmd docs will determine their numbers in the report. This might result in Table 2 being referenced before Table 1, for instance, in your report.
...
However, we understand that some will strongly prefer to set the order of their tables and figures. You *can* try manually changing the order of the figure and table chunks in the 08_figures.qmd and 09_tables.qmd docs, but this isn't a reproducible or permanent solution.
