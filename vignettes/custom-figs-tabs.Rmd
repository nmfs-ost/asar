...
## Make an figures_tables folder (if needed)

Do you already have an figures_tables folder? *Hint: it should be in your working directory.* If yes: Great! You can skip this step. If not, simply enter the code below to make an figures_tables folder.

```{r eval=FALSE}
dir.create(fs::path(getwd(), "figures_tables"))
```

## Export your rdas

> **NOTE**: Your exported rda file must end in "table" or "figure" (e.g., "example_tab_table.rda", "example_fig_figure.rda"). If it does not, the table or figure will not be added to your Figures or Tables documents.

### Tables

This will produce an rda called "example_tab_table.rda" and save it in your figures_tables folder.

```{r eval=FALSE}
save(rda,
  file = fs::path(
    getwd(), # this is your figures_tables directory
    "figures_tables",
    paste0(
...
This will produce an rda called "example_fig_figure.rda" and save it in your figures_tables folder.
...
figures_tables_dir <- getwd() # replace this with the location of your figures_tables folder
...
You can check out your split tables in the new rda saved in your figures_tables_dir folder. It will have the same filename as your original rda except that it will contain "\_split" in the filename. For instance, split tables created from "example_tab_table.rda" will be saved in "example_tab_table_split.rda".
...
In your tables doc, add your first R chunk (shown below). This chunk is used to establish your figures_tables_dir folder and load the library needed to plot your tables (`flextable`).
...
#| include: false
library(flextable)
figures_tables_dir <- "C:/path/to/your/figures_tables" # set the path to your figures_tables_dir
``` 
````
...
load(file.path(figures_tables_dir, "example_tab_table.rda"))
...
load(file.path(figures_tables_dir, "example_table_split.rda"))
...
In your figures doc, add your first R chunk (shown below). This chunk is used to establish your figures_tables_dir folder.
...
load(file.path(figures_tables_dir, "example_fig_figure.rda"))
