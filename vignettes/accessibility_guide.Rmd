...
*Date updated: 9/15/2025*

## Tagging ⚠️

### PDFs

`add_tagging()` adds tags to *some* PDFs. Keep reading for an explanation.

#### What isn't working

1. Tables
  - **Report compilation will fail unless the report does not contain tables.**
  - More context: Tagging tables is a known issue among developers. It's very challenging.
  - In the meantime, we suggest two workarounds: 1) placing your tables in a separate document, or 2) adding your tables as raw LaTeX code inside the 08_tables.qmd file. We are working on adding option #2 as a feature in a future `asar` release.
...
`add_alttext()` is currently failing to add alternative text to images, though *it was successful in the past*. We are investigating why this feature is failing.
...
The glossary is fully functional. 

## Evaluation of criteria in Section 508 compliance checklist
