...
#' years <- c("1984-1999 (triennial), 2001-2013 (biennial)","1984, 1987, 1990, 1993, 1996, 1999, 2003, 2005, 2007, 2009, 2011","1961-2013", "1990,1998-2002, 2004, 2005, 2006, 2008, 2010","1963-1977, 1991-1997")
#' test_obj <- data.frame(source, data, years)
#'
#' export_object(object = test_obj, file_format = "csv")
#'
...
export_object <- function(
  ...
  if(file_format=="pdf"){
    ...
    grDevices::pdf(file = paste0(here::here('exported'),"/",sprintf(deparse(substitute(object))),".pdf")) # pdf fxn in base r
    ...
    print(doc_exp,
          target = paste0(here::here('exported'),"/", sprintf(deparse(substitute(object))),".docx")) # %>% invisible()

  } else if(file_format=="xlsx"){

    openxlsx::write.xlsx(object,
                         file = paste0(here::here('exported'),"/", sprintf(deparse(substitute(object))), ".xlsx"),
                         overwrite = FALSE)

  } else if(file_format=="csv"){

    utils::write.csv(object,
              file = paste0(here::here('exported'),"/", sprintf(deparse(substitute(object))), ".csv"),
              row.names = FALSE)

  } else if(file_format=="rdata"){

    save(object,
         file = paste0(here::here('exported'), "/", sprintf(deparse(substitute(object))), ".Rdata"))
