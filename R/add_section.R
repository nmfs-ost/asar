...
add_section <- function(
  ...
  for (i in 1:length(new_section)) {
    if (custom & is.null(custom_sections)) {
      ...
      # extract section from argument list
      section_i_name <- paste0(gsub(" ", "_", tolower(new_section[i])), ".qmd")
      # Format text to be made into a quarto doc
      section_i <- paste0(
      ...
      # save text as quarto doc in local folder
      utils::capture.output(cat(section_i), file = paste0(subdir, "/", section_i_name), append = FALSE)

      # Keeping below code for future [TODO]
      # The last line before next part of if statement will need to change as well
      ...
      #   child_sec <- add_child(
      #     section_i_name,
      #     label = gsub(" ", "_", tolower(new_section[2]))
      #   )
      ...
      if (locality == "before") {
        sectioning <- append(custom_sections, section_i_name, after = (which(grepl(local_section, custom_sections)) - 1))
      } else if (locality == "after") {
        if (locality %in% locality_prev & local_section %in% local_section_prev) {
          sectioning <- append(custom_sections, section_i_name, after = which(grepl(gsub(" ", "_", tolower(new_section[i - 1])), custom_sections)))
        } else {
          sectioning <- append(
            custom_sections,
            section_i_name,
